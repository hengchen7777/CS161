...
(defun at-most-one-color (n c k)
	(cond ((= c k) nil)
		(t (append (negative-pairs-basic (- 0 (node2var n c k)) n c k) (at-most-one-color n (+ c 1) k)))
	)
)

; helper fuction for at-most-one-color
; negative-pairs-basic takes a negative integer x, a node index n, 
; a color index c and the max color index k. It returns a list of 
; negative pairs with the first element set to x, and the second
; element is set to the negative value of n set to the color (c+1, c+2, ...k)
(defun negative-pairs-basic (x n c k)
	(cond ((= c k) nil)
		(t (cons (list x (- 0 (node2var n (+ c 1) k))) (negative-pairs-basic x n (+ c 1) k)))
	)
)
...
(defun generate-edge-clauses (e k)
 (conflicting-pairs (first e) (second e) 1 k) 
)

;helper function for generate-edge-clauses
;conflicting-pairs takes in two nodes x and y, a color index c, and the max color index k,
;and it returns a list of negative pairs of x y with color form the set (1,2,...,k).
(defun conflicting-pairs (x y c k)
	(cond ((> c k) nil)
		(t (cons (list (- 0 (node2var x c k)) (- 0 (node2var y c k))) (conflicting-pairs x y (+ c 1) k)))
	)
)
